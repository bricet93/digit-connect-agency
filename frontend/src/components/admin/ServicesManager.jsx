...
export default function ServicesManager() {
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-8 shadow-xs text-left">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-digitBlue hover:bg-digitBlue/90 text-white font-bold text-xs px-4 py-2.5 rounded-lg shadow-md cursor-pointer"
        >
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-lg max-w-lg w-full p-6 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSave} className="space-y-3 text-xs">
                ...
                <div>
                  ...
                  <input type="text" required value={formData.title_fr} onChange={(e) => setFormData({ ...formData, title_fr: e.target.value })} className="w-full p-2.5 bg-slate-50 border rounded-lg" />
                </div>
                <div>
                  <label className="block font-semibold text-slate-600 mb-1">Titre (EN)</label>
                  <input type="text" value={formData.title_en} onChange={(e) => setFormData({ ...formData, title_en: e.target.value })} className="w-full p-2.5 bg-slate-50 border rounded-lg" />
                </div>
                <div>
                  <label className="block font-semibold text-slate-600 mb-1">Description (FR)</label>
                  <textarea value={formData.description_fr} onChange={(e) => setFormData({ ...formData, description_fr: e.target.value })} className="w-full p-2.5 bg-slate-50 border rounded-lg" rows={2} />
                </div>
                <div>
                  <label className="block font-semibold text-slate-600 mb-1">Description (EN)</label>
                  <textarea value={formData.description_en} onChange={(e) => setFormData({ ...formData, description_en: e.target.value })} className="w-full p-2.5 bg-slate-50 border rounded-lg" rows={2} />
                </div>
                ...
                <div className="flex justify-end gap-2 pt-4 border-t">
                  <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 bg-slate-100 rounded-lg">Annuler</button>
                  <button type="submit" className="px-5 py-2 bg-digitBlue text-white font-bold rounded-lg">Enregistrer</button>
                </div>
              </form>
            </motion.div>
          </div>
