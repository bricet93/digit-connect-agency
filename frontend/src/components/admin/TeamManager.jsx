    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-8 text-left">
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-digitBlue hover:bg-digitPink text-white font-bold text-xs px-4 py-2.5 rounded-xl transition-colors cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {members.map(item => (
          <div key={item.id} className="border border-slate-200 rounded-2xl p-4 flex gap-4 items-center bg-slate-50/50 hover:bg-white hover:shadow-md transition-all">
            <img 
            ...
            <div className="flex flex-col gap-1">
              <button onClick={() => handleOpenModal(item)} className="p-2 text-slate-600 bg-white border border-slate-200 hover:text-digitBlue rounded-xl transition-colors cursor-pointer">
                <Edit2 size={14} />
              </button>
              <button onClick={() => handleDeleteClick(item)} className="p-2 text-rose-600 bg-rose-50 border border-rose-100 hover:bg-rose-100 rounded-xl transition-colors cursor-pointer">
                <Trash2 size={14} />
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
              ...
              <button onClick={() => setShowModal(false)} className="p-2 text-slate-400 hover:text-slate-600 rounded-xl">
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-9 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-digitBlue outline-none" 
                  />
                </div>
              </div>
              ...
              <div className="border border-slate-200 p-4 rounded-2xl bg-slate-50/70 space-y-3">
                ...
                {uploadMode === 'url' ? (
                  <input 
                    ...
                    className="w-full p-2.5 border border-slate-200 rounded-xl bg-white focus:border-digitBlue outline-none" 
                  />
                ) : (
                  <input 
                    type="file" 
                    ...
                    className="w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-digitBlue file:text-white file:font-bold cursor-pointer" 
                  />
                )}

                {formData.photo_url && (
                  <div className="flex items-center gap-3 pt-2">
                    <img src={formData.photo_url} alt="Aperçu" className="w-12 h-12 rounded-xl object-cover border" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                ...
                <div>
                  ...
                  <button
                    ...
                    className={`w-full py-2.5 px-4 rounded-xl font-bold flex items-center justify-center gap-2 border transition-all cursor-pointer ${
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
                ...
                <button 
                  type="submit" 
                  className="px-6 py-2.5 bg-digitBlue hover:bg-digitPink text-white font-bold rounded-xl transition-colors shadow-md cursor-pointer"
                >
